...
function ChatPage() {
  ...
  // ADDITION 1: We told React that messages can have an optional array of "sources"
  ...
  const askBrain = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ADDITION 2: Now we also save data.sources in the bot's message!
      setMessages((prev) => [
        ...prev,
        {
          role: "bot",
          content: data.answer,
          sources: data.sources, // <-- Here they are!
        },
      ]);
    } catch (err: any) {
      setMessages((prev) => [
...
// 2. 3D Visualizer Page (Updated for Legend and Colors)
function MapPage() {
  const [mapData, setMapData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [view, setView] = useState<"2d" | "3d" | "graph">("2d"); // New Tabs management
  const [showLabels, setShowLabels] = useState(false); // State to show/hide the labels
  ...
  const plotly3D = mapData.traces3D.map((trace: any) => ({
    ...
    textfont: { size: 10, color: "#fff" }, // Improved text readability
    ...
    <div className="flex-1 flex flex-col p-6 h-full">
      <div className="flex justify-between items-end mb-4">
        ...
        <div className="flex items-center gap-4">
          {/* LABELS BUTTON (Visible only in 2D or 3D) */}
          {(view === "2d" || view === "3d") && (
            <button
...
function EditorPage() {
  ...
  const executeCommand = async (cmd: any) => {
    ...
    if (!cmd.type || cmd.type === "format") {
      ...
      // If there was some text selected before the slash (a bit hacky, but let's try)
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (showSlashMenu) {
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault();
        executeCommand(filteredCommands[selectedIndex]); // <--- Updated here
  ...
  // New states for loading the notes
  ...
  const handleContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    // Logic to show the menu: if the last character or the current word starts with /
    const words = val.split(/(\s+)/);
    const lastWord = words[words.length - 1];

    if (lastWord.startsWith("/")) {
      setShowSlashMenu(true);
      setSlashQuery(lastWord.substring(1)); // Takes everything after the slash to filter
      setSelectedIndex(0);
    } else {
      setShowSlashMenu(false);
    }
  ...
  // 4. Drag & Drop handling for Images/Files
  ...
  const handleDrop = async (e: React.DragEvent<HTMLTextAreaElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      // Check whether it is a supported file or image
  ...
  // 1. Load the list of notes when the editor opens
  useEffect(() => {
    fetch(`${API_URL}/notes`)
      .then((res) => res.json())
      .then((data) => setAvailableNotes(data.notes || []))
      .catch(console.error);
  }, [status]); // Reload the list every time we save a new note
  ...
  const deleteAttachment = async (fullMatch: string, filename: string) => {
    ...
    try {
      ...
      if (res.ok) {
        // Remove the markdown syntax from the text block
  ...
  // 2. Function to load a specific note
  const loadNote = async (filename: string) => {
    try {
      ...
      // Temporarily block auto-save to avoid accidental overwrites on load
      setIsTyping(false);
      setTitle(data.title);
  ...
  useEffect(() => {
    if (!isTyping) return; // Don't save if we just loaded a file
        ...
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          {status === "saved" && (
            <span className="text-emerald-400">
              All changes saved to Raw Notes
            </span>
          )}
          {status === "idle" && (
            <span className="text-slate-500">Ready in raw_notes/</span>
          )}
          {status === "error" && (
        ...
        >
          <div className="max-w-7xl mx-auto flex flex-col h-full gap-6 relative">
            ...
            <div className="relative flex-1 flex flex-col lg:flex-row gap-8 overflow-hidden pb-6">
              ...
              {/* PREVIEW PANEL (RENDER MARKDOWN + LARGE IMAGES) RENDERED ONLY IN SPLIT OR PREVIEW */}
