...
function ChatPage() {
  ...
  const askBrain = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {role: 'bot', content: `Errore: ${err.message}`}]);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-1 overflow-y-auto mb-4 space-y-6 scrollbar-hide p-2 md:p-6">
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-slate-500 gap-4">
            ...
            <p className="text-lg text-center">Fai una domanda al tuo Second Brain.<br/><span className="text-sm">Risponderà usando solo i tuoi appunti.</span></p>
          </div>
        )}
...
function EditorPage() {
  ...
  const insertCommand = (syntax: string) => {
    // Trova l'ultima occorrenza di "/" prima della fine del testo attuale
    const lastSlashIndex = content.lastIndexOf("/");
    const newContent = content.substring(0, lastSlashIndex) + syntax;
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (showSlashMenu) {
      if (e.key === "ArrowDown") {
        ...
        e.preventDefault();
        insertCommand(filteredCommands[selectedIndex].syntax);
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-950 text-slate-200">
      ...
      {/* AREA EDITOR */}
      <div className="flex-1 overflow-y-auto px-6 py-12 md:px-24 lg:px-48 relative">
        <div className="max-w-3xl mx-auto flex flex-col h-full gap-6 relative">
          {/* TITOLO */}
          <input
            type="text"
            value={title}
            ...
            className="w-full bg-transparent text-4xl md:text-5xl font-bold text-slate-100 placeholder-slate-700 outline-none border-none focus:ring-0"
          />
          ...
          <div className="relative flex-1 flex flex-col">
            <textarea
              ...
              className="w-full flex-1 bg-transparent text-lg text-slate-300 placeholder-slate-700 outline-none border-none focus:ring-0 resize-none leading-relaxed"
            />

            {/* MENU SUGGERIMENTI STILE NOTION */}
            {showSlashMenu && filteredCommands.length > 0 && (
              <div className="absolute top-10 left-0 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-[100] overflow-hidden">
                ...
                {filteredCommands.map((cmd, index) => (
                  ...
                  >
                    <span className="w-6 h-6 flex items-center justify-center bg-slate-800 rounded text-xs font-bold">
                      {cmd.icon}
                    </span>
